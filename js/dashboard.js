...
(function () {
  ...
  // Lock Test Flight & Schedule Mission (Also handles Updating Modified Chapters)
  ...
  if (btnScheduleTest) {
    btnScheduleTest.addEventListener('click', () => {
      ...
      if (selectedChapters.length === 0) {
        ...
        showToast('⚠️ Please tick at least 1 chapter to cover!');
      ...
      if (isModifyMode) {
        ...
        try {
          localStorage.setItem('preppilot_flight_mission_' + userSanitized, JSON.stringify(updatedMission));
        } catch (err) {
          console.error('Error saving updated mission:', err);
        }
        ...
        showToast(`✅ Test flight updated! Now targeting <strong>${selectedChapters.length} chapters</strong> for <strong>${formatDateHuman(updatedMission.testDate)}</strong>.`);
        return;
      }
      ...
      try {
        localStorage.setItem('preppilot_flight_mission_' + userSanitized, JSON.stringify(scheduledMission));
      } catch (err) {
        console.error('Error saving scheduled mission:', err);
      }
      ...
      showToast(`🎯 Test flight locked for <strong>${subjData.name}</strong> on <strong>${formatDateHuman(scheduledMission.testDate)}</strong>! Daily Pilot Planner and Study Kit ready.`);
    });
  }

  // Render Active Scheduled Mission Banner Card
    ...
    wrap.querySelectorAll('.btn-remove-ch-tag').forEach((btn) => {
      btn.addEventListener('click', (e) => {
        ...
        if (idx >= 0) {
          ...
          try {
            localStorage.setItem('preppilot_flight_mission_' + userSanitized, JSON.stringify(mission));
          } catch (err) {
  ...
  // Restore saved mission on load
  try {
    const savedMissionRaw = localStorage.getItem('preppilot_flight_mission_' + userSanitized);
    if (savedMissionRaw) {
      const savedMission = JSON.parse(savedMissionRaw);
      if (savedMission && savedMission.chapters && savedMission.chapters.length > 0) {
        renderActiveMissionCard(savedMission);

        // Auto-launch modify mode if ?modify=true in URL
        if (urlParams.get('modify') === 'true') {
          setTimeout(() => {
            startModifyMission(savedMission);
          }, 350);
        }
      }
    }
  } catch (err) {
    console.error('Failed to load saved flight mission:', err);
  }
  ...
  if (btnConfirmSignout) {
    btnConfirmSignout.addEventListener('click', () => {
      // Return to login page (landing page auth cockpit)
